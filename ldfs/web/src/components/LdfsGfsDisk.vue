<template>
  <div>
    <span class="raid-text">集群磁盘管理</span>
    </br>
    <input type="button" value="查询" class="mo-btn-x mo-btn-gray AddBtn" @click="refresh">
    <div class="scope-content-select">
      <el-select v-model="currentip" placeholder="placeholder" @change="refresh">
        <el-option
...
<script>
  export default {
    ...
    data() {
      return {
        currentip: "172.16.83.222",
        tableData: [],
        peers:["172.16.83.222","172.16.83.224"],
      }
    },
    created() {
      this.init();
    ...
    methods: {
      init(flash) {
        ...
        this.axios.get('http://'+this.currentip+':8019/ldfs/api/v1/disk').then(
          res => {
      ...
      stopAndCancel(index) {
        ...
        this.$prompt.instance.$on("detemine", () => {
          ...
          this.axios.delete('http://'+this.currentip+':8019/ldfs/api/v1/disk', {data: {'id': this.tableData[index].id, "slot": this.tableData[index].slot}}).then(res => {
            this.$prompt.instance.promptClose();
            if (res.data.code===0) {
              this.$successPrompt.success(
                {
      ...
      addDisk(index) {
        ...
        this.$prompt.instance.$on("detemine", () => {
          ...
          this.axios.post('http://'+this.currentip+':8019/ldfs/api/v1/disk', {'id': this.tableData[index].id, "slot": this.tableData[index].slot}).then(res => {
            this.$prompt.instance.promptClose();
            if (res.data.code===0) {
              this.$successPrompt.success(
                {
