...
from kafkao.vendor import six  # pylint: disable=import-error

import kafkao.protocol.commit
import kafkao.protocol.fetch
import kafkao.protocol.message
import kafkao.protocol.metadata
import kafkao.protocol.offset
import kafkao.protocol.produce
import kafkao.structs

from kafkao.codec import gzip_encode, snappy_encode
from kafkao.errors import ProtocolError, UnsupportedCodecError
from kafkao.structs import ConsumerMetadataResponse
from kafkao.util import (
...
class KafkaProtocol(object):
    ...
    def encode_produce_request(cls, payloads=(), acks=1, timeout=1000):
        ...
        return kafkao.protocol.produce.ProduceRequest[0](
            required_acks=acks,
            timeout=timeout,
            topics=topics
        )
    ...
    def decode_produce_response(cls, response):
        ...
        """
        return [
            kafkao.structs.ProduceResponsePayload(topic, partition, error, offset)
    ...
    def decode_fetch_response(cls, response):
        ...
        """
        return [
            kafkao.structs.FetchResponsePayload(
    ...
    def decode_message_set(cls, messages):
        for offset, _, message in messages:
            if isinstance(message, kafkao.protocol.message.Message) and message.is_compressed():
                inner_messages = message.decompress()
                for (inner_offset, _msg_size, inner_msg) in inner_messages:
                    yield kafkao.structs.OffsetAndMessage(inner_offset, inner_msg)
            else:
                yield kafkao.structs.OffsetAndMessage(offset, message)

    @classmethod
    def encode_offset_request(cls, payloads=()):
        return kafkao.protocol.offset.OffsetRequest[0](
            replica_id=-1,
            topics=[(
                topic,
                [(
    ...
    def decode_offset_response(cls, response):
        ...
        """
        return [
            kafkao.structs.OffsetResponsePayload(topic, partition, error, tuple(offsets))
            for topic, partitions in response.topics
    ...
    def decode_list_offset_response(cls, response):
        ...
        """
        return [
            kafkao.structs.ListOffsetResponsePayload(topic, partition, error, timestamp, offset)
            for topic, partitions in response.topics
    ...
    def encode_metadata_request(cls, topics=(), payloads=None):
        ...
        return kafkao.protocol.metadata.MetadataRequest[0](topics)
    ...
    def decode_offset_commit_response(cls, response):
        ...
        """
        return [
            kafkao.structs.OffsetCommitResponsePayload(topic, partition, error)
            for topic, partitions in response.topics
    ...
    def decode_offset_fetch_response(cls, response):
        ...
        """
        return [
            kafkao.structs.OffsetFetchResponsePayload(
...
def create_message(payload, key=None):
    ...
    """
    return kafkao.structs.Message(0, 0, key, payload)
...
def create_gzip_message(payloads, key=None, compresslevel=None):
    ...
    return kafkao.structs.Message(0, 0x00 | codec, key, gzipped)
...
def create_snappy_message(payloads, key=None):
    ...
    return kafkao.structs.Message(0, 0x00 | codec, key, snapped)
